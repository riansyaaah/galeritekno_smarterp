...
class ModelReceiptvoucher extends CI_Model
{
    ...
    public function getReceiptvoucherdetail($where = '')
    {
    ...
    public function getReceiptvoucher($VoucherNo)
    {
        return $this->db->query("SELECT max(SUBSTR(VoucherNo, 1, 3)) as VoucherNo FROM fin_voucher where VoucherNo like '%$VoucherNo' and VoucherType = '1'")->result_array();
    }
    ...
    public function getReceiptvoucherForPrint($id)
    {
        $this->db->select('*');
    ...
    public function getReceiptvoucherdetailForPrint($id)
    {
        $this->db->select('*');
    ...
    {
        // Harusnya "debit" saya rasa, tapi di table masuk ke Kredit :D
        $this->db->select_sum('Credit', 'Credit_total');
        $this->db->where('VoucherNo', $id);
