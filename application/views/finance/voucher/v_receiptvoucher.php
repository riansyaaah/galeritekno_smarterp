...
<body>
							...
							<div class="card">
								...
								<div class="card-body row">
									...
									<div class="col-lg-6">
										...
										<div class="form-group row">
											<label for="example-text-input" class="col-sm-3 col-form-label text-right">Receipt No:</label>
											...
											<label for="example-text-input" class="col-sm-3 col-form-label text-right">Receipt Date:</label>
											<div class="col-sm-9 mb-1">
									...
									<div class="col-lg-2">
										<div class="form-group">
											<button type="button" class="btn  btn-primary btn-sm btn-block my-1 text-left" id="btn_new_Receipt" disabled onclick="newReceipt()"><i class="fa fa-file px-2"></i> New Receipt</button>
											<button type="button" class="btn  btn-primary btn-sm btn-block my-1 text-left" id="btn_edit_Receipt" disabled onclick="editReceipt()"><i class="fa fa-edit px-2"></i> Edit Receipt</button>
											<button type="button" class="btn  btn-primary btn-sm btn-block my-1 text-left" id="btn_delete_Receipt" onclick="return ReceiptHapus()" disabled><i class="fa fa-trash px-2"></i> Delete Receipt</button>
											<button type="button" class="btn  btn-primary btn-sm btn-block my-1 text-left" id="btn_save_Receipt" onclick="return ReceiptBaru()" disabled><i class="fa fa-save px-2"></i> Save Receipt</button>
											<!-- <button type="button" class="btn  btn-primary btn-sm btn-block my-1 text-left" id="btn_print_Receipt" disabled><i class="fa fa-print px-2"></i> Print Receipt</button> -->
											<!-- Print -->
											<?php $attributes = array('target' => '_blank'); ?>
											<?= form_open('finance/voucher/receiptvoucher/cetak', $attributes); ?>
											<input type="hidden" id="printVoucherNo" name="printVoucherNo">
											<input type="hidden" id="printSumAmount" name="printSumAmount">
											<button type="submit" class="btn  btn-primary btn-sm btn-block my-1 text-left" id="btn_print_Receipt" disabled><span class="text-left"><i class="fa fa-print px-2"></i></span> Print Receipt</button>
											<?php echo form_close() ?>
											<!-- End Print -->
											<button type="button" class="btn  btn-primary btn-sm btn-block my-1 text-left" id="btn_selesai_Receipt" onclick="selesai()" disabled><span class="text-left"><i class="fa fa-check px-2"></i></span> Selesai</button>
										</div>
								...
								<div class="card-body">
									<div class="table-responsive">
										<button type="button" class="btn  btn-primary btn-sm" id="btn_get_reimbursment" disabled hidden>Get Sales Invoice</button>
										<button type="button" class="btn  btn-primary btn-sm my-2" id="btn_add_item" onclick="addItem()" disabled> <i class="fa fa-plus"></i> Add Items</button>
										<table class="table table-striped" id="table-detail-Receipt">
											<thead>
												<tr>
													<th>NO</th>
													<th>DESCRIPTION</th>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<table class="table table-bordered table-hover" id="table-pycashbank" style="width:100%;">
						<thead>
							<tr>
								<th class="text-center">Receipt Voucher NO</th>
								<th class="text-center">Receipt Voucher Date</th>
	...
	<div class="modal fade" id="ReceiptHapus" role="dialog" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				...
				<div class="modal-footer bg-whitesmoke br">
					<button class="btn btn-primary" type="button" id="btnReceiptHapus" onclick="return HapusReceipt()"> Hapus </button>
					<button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
				</div>
			</div>
		</div>
	...
	<div class="modal fade" id="addItem" role="dialog" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Input Item Receipt</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control form-control-sm" id="itemCredit" required="">
					</div>
					<div class="form-group row">
						<div class="col-md-6">
	...
	<script>
		const rupiah = document.querySelector('#itemCredit');
		...
		function cariCashbank() {
			...
			$("#table-menu").dataTable({
				destroy: true,
				ajax: {
					url: '<?php echo base_url("finance/voucher/Receiptvoucher/getCashbank") ?>',
					dataSrc: 'data'
				},
				columns: [{
						"data": 'BankCode',
			...
			$('#table-menu tbody').on('click', 'tr', function() {
				...
				document.getElementById("VoucherNo").value = '/' + Kode_Bank + "/D/<?= $PeriodeActive; ?>";
				document.getElementById("btn_new_Receipt").disabled = false;
				...
				document.getElementById("btn_save_Receipt").disabled = true;
				document.getElementById("btn_delete_Receipt").disabled = true;
				document.getElementById("btn_print_Receipt").disabled = true;
				document.getElementById("btn_selesai_Receipt").disabled = true;
				...
				// flash Table item
				$('#table-detail-Receipt').DataTable().clear().draw();
				$('#table-detail-Receipt').DataTable().destroy();
				$('#totalAmount').text('');
				$('#totalAmountLable').text('');
		...
		function cariPYCashbank() {
			...
			$("#table-pycashbank").dataTable({
				destroy: true,
				ajax: {
					url: '<?php echo base_url("finance/voucher/Receiptvoucher/getPYCashbank") ?>',
					dataSrc: 'data',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
		...
		function newReceipt() {
			...
			$.ajax({
				type: 'POST',
				url: '<?= base_url("finance/voucher/Receiptvoucher/cariVoucherNo"); ?>',
				dataType: 'json',
				data: dataPost,
				success: function(result) {
					dismisLoading();
					document.getElementById("VoucherNo").value = result['VoucherNo'] + '/' + BankCode + "/D/<?= $PeriodeActive; ?>";
					...
					document.getElementById("btn_save_Receipt").disabled = false;
					document.getElementById("btn_edit_Receipt").disabled = true;
		...
		function ReceiptBaru() {
			var btn = document.getElementById("btn_save_Receipt");
			...
			if (VoucherDate == "" || VoucherDate == null) {
				...
				$.ajax({
					url: '<?php echo base_url("finance/voucher/Receiptvoucher/addReceipt") ?>',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
					success: function(res) {
						...
						if (res.status_json) {
							...
							btn.value = '<i class="fa fa-save px-2"></i> Save Receipt';
							btn.innerHTML = '<i class="fa fa-save px-2"></i> Save Receipt';
							document.getElementById("btn_selesai_Receipt").disabled = false;
		...
		function ReceiptHapus() {
			...
			showLoading();
			$.ajax({
				url: '<?php echo base_url("finance/voucher/Receiptvoucher/getReceiptvoucherbyid") ?>',
				type: 'POST',
				dataType: 'json',
				data: dataPost,
				success: function(res) {
		...
		function HapusReceipt() {
			...
			$.ajax({
				url: '<?php echo base_url("finance/voucher/Receiptvoucher/hapusReceipt") ?>',
				type: 'POST',
				dataType: 'json',
				data: dataPost,
				success: function(res) {
		...
		function cariAccountNo() {
			...
			$("#table-account").dataTable({
				destroy: true,
				ajax: {
					url: '<?php echo base_url("finance/voucher/Receiptvoucher/getAccountNo") ?>',
					dataSrc: 'data'
				},
				columns: [{
						"data": 'no',
			...
			$.ajax({
				...
				success: function(res) {
					...
					if (res.status_json) {
						...
						$("#table-detail-Receipt").dataTable({
							paging: false,
							ordering: false,
							info: false,
							searching: false,
							destroy: true,
							ajax: {
								url: '<?php echo base_url("finance/voucher/Receiptvoucher/caridetailReceiptvoucher") ?>',
								dataSrc: 'data',
								type: 'POST',
								dataType: 'json',
								data: dataPost,
						...
						document.getElementById("btn_save_Receipt").disabled = true;
						document.getElementById("btn_edit_Receipt").disabled = false;
						document.getElementById("btn_delete_Receipt").disabled = false;
						document.getElementById("btn_print_Receipt").disabled = false;
		...
		function editItem(idrow) {
			...
			showLoading();
			$.ajax({
				url: '<?php echo base_url("finance/voucher/Receiptvoucher/getItemvoucher") ?>',
				type: 'POST',
				dataType: 'json',
				data: dataPost,
				success: function(res) {
		...
		function itemBaru() {
			...
			if (itemDescription == "" || itemDescription == null) {
				...
				$.ajax({
					url: '<?php echo base_url("finance/voucher/Receiptvoucher/addItem") ?>',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
					success: function(res) {
						...
						if (res.status_json) {
							...
							document.getElementById("itemCredit").value = "";
		...
		function itemHapus() {
			...
			$.ajax({
				url: '<?php echo base_url("finance/voucher/Receiptvoucher/hapusItemvoucher") ?>',
				type: 'POST',
				dataType: 'json',
				data: dataPost,
				success: function(res) {
