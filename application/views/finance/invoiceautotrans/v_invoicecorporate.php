...
<body>
							...
							<div class="card">
								...
								<div class="card-body row">
									<div class="col-lg-12">
										<div class="form-group row">
											<div class="col-md-4">
												<label for="example-text-input">Invoice Corporate Date:</label>
												<div class="input-group">
													<input class="form-control form-control-sm" id="InvoiceCorDate" type="date">
												</div><br>
												<label for="InvoiceCorNo">Invoice Corporate No</label>
												<div class="input-group mb-3">
													<input type="text" id="InvoiceCorNo" name="example-input2-group2" class="form-control form-control-sm" readonly>
													<span class="input-group-append">
														<a href="#" class="edit_record btn btn-primary btn-sm" id="btnCariInvoiceCor"><i class="fas fa-list-ul"></i></a>
													</span>
												</div>
											</div>
										</div>
								...
								<div class="card-body">
									<div class="table-responsive">
										...
										<button type="button" class="btn  btn-primary btn-sm" id="btnNewInvoiceCor"><i class="fa fa-file"></i> New Invoice</button>&nbsp;
										<button type="button" class="btn  btn-primary btn-sm" id="btn_edit_payment"><i class="fa fa-edit"></i> Edit Invoice</button>&nbsp;
										<button type="button" class="btn  btn-primary btn-sm" id="btn_delete_payment"><i class="fa fa-trash"></i> Delete Invoice</button>&nbsp;
	...
	<div class="modal fade" id="cariInvoiceCor" role="dialog" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered modal-xl" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Invoice Corporate</h5>
				...
				<div class="modal-body">
					<table class="table table-bordered table-hover" id="table-menu" style="width:100%;">
						<thead>
							<tr>
								<th class="text-center">Invoice Corporate Date</th>
								<th class="text-center">Invoice Corporate No</th>
	...
	<div class="modal fade" id="addItem" role="dialog" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Input Item Invoice Corporate</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
	...
	<script>
		...
		$(document).ready(function() {
			$("#InvoiceCorDate").change(function() {
				$("#InvoiceCorDate").removeClass("is-invalid");
			});

			// Efek Terhadapt Tombol					
			document.getElementById("btnNewInvoiceCor").disabled = false;
		...
		function reset() {
			...
			// Efek Terhadap Form
			document.getElementById("InvoiceCorDate").readOnly = false;
			document.getElementById("InvoiceCorDate").value = '';
			document.getElementById("InvoiceCorNo").value = '';

			// Efek Terhadapt Tombol					
			document.getElementById("btnReset").hidden = true;
			document.getElementById("btnNewInvoiceCor").hidden = false;
			document.getElementById("btnNewInvoiceCor").disabled = false;
			document.getElementById("btn_edit_payment").disabled = true;
			document.getElementById("btn_delete_payment").disabled = true;
			document.getElementById("btnPrintPayment").disabled = true;
		...
		document.querySelector('#btnCariInvoiceCor').addEventListener('click', e => {
			showLoading();
			$("#table-menu").dataTable({
				destroy: true,
				ajax: {
					url: "<?= base_url('finance/invoiceautotrans/invoicecorporate/getInvoiceCor') ?>",
					dataSrc: 'data'
				},
				columns: [{
						"data": 'InvoiceCorDate',
			...
			dismisLoading();
			$("#cariInvoiceCor").modal();
			$('#table-menu tbody').on('click', 'tr', function() {
				event.preventDefault();
				let $td = $(this).closest('tr').children('td');
				let InvoiceCorDate = $td.eq(0).text();
				let InvoiceCorNo = $td.eq(1).text();
				document.getElementById("InvoiceCorDate").value = InvoiceCorDate;
				document.getElementById("InvoiceCorNo").value = InvoiceCorNo;
				InvoiceCorCari();
				$('#cariInvoiceCor').modal('hide');

				// Efek terhadap Form

				document.getElementById("InvoiceCorDate").readOnly = true;
				document.getElementById("InvoiceCorNo").readOnly = true;

				// Efek Terhadapt Tombol					
				document.getElementById("btnNewInvoiceCor").hidden = true;
		...
		document.querySelector('#btnNewInvoiceCor').addEventListener('click', e => {
			const InvoiceCorDate = $('#InvoiceCorDate').val();
			if (InvoiceCorDate == '' || InvoiceCorDate == null) {
				showSnackError("Harap isi Tanggal Terlebih Dahulu");
				$("#InvoiceCorDate").addClass("is-invalid");
			} else {
				$.ajax({
					type: 'POST',
					url: '<?= base_url("finance/invoiceautotrans/invoicecorporate/cariInvoiceCorNo"); ?>',
					dataType: 'json',
					success: result => {
						dismisLoading();
						document.querySelector('#InvoiceCorNo').value = `${result['InvoiceCorNo']}/IC/<?= $tahunaktif; ?>`;
						document.querySelector('#InvoiceCorDate').readOnly = true;
		...
		document.querySelector('#btn_addItem').addEventListener('click', e => {
			showLoading();
			dismisLoading();
			const InvoiceCorNo = $('#InvoiceCorNo').val();
			const InvoiceCorDate = $('#InvoiceCorDate').val();
			if (InvoiceCorDate == '' || InvoiceCorDate == null) {
				showSnackError("Harap isi Invoice Corporate Date");
			} else if (InvoiceCorNo == '' || InvoiceCorNo == null) {
				showSnackError("Harap isi Invoice Corporate No");
			} else {
				$("#addItem").modal();
		...
		document.querySelector('#btnCariAccountNo').addEventListener('click', e => {
			...
			$("#table-account").dataTable({
				destroy: true,
				ajax: {
					url: '<?= base_url("finance/invoiceautotrans/invoicecorporate/getAccountNo") ?>',
					dataSrc: 'data'
				},
				columns: [{
						"data": 'no',
		...
		function itemBaru() {
			...
			if (itemDescription == '' || itemDescription == null) {
				...
				$.ajax({
					url: '<?= base_url("finance/invoiceautotrans/invoicecorporate/addItem") ?>',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
		...
		function editItem(idrow) {
			...
			showLoading();
			$.ajax({
				url: '<?php echo base_url("finance/invoiceautotrans/invoicecorporate/getItemInvoiceCor") ?>',
				type: 'POST',
				dataType: 'json',
				data: dataPost,
				success: function(res) {
		...
		function itemHapus() {
			...
			$.ajax({
				url: '<?php echo base_url("finance/invoiceautotrans/invoicecorporate/hapusItemInvoiceCor") ?>',
				type: 'POST',
				dataType: 'json',
				data: dataPost,
				success: function(res) {
		...
		function InvoiceCorCari() {
			showLoading();
			InvoiceCorNo = document.querySelector('#InvoiceCorNo').value;
			dataPost = {
				InvoiceCorNo: InvoiceCorNo,
			}
			$("#table-detail-payment").dataTable({
				destroy: true,
				ajax: {
					url: '<?= base_url("finance/invoiceautotrans/invoicecorporate/caridetailInvoiceCor") ?>',
					dataSrc: 'data',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
		...
		document.querySelector('#btnPrintPayment').addEventListener('click', e => {
			...
			dismisLoading();
			const InvoiceCorNo = $('#InvoiceCorNo').val();
			if (InvoiceCorNo == '' || InvoiceCorNo == null) {
				showSnackError("Harap isi Invoice Corporate No");
			} else {
				fetch(`<?= base_url('finance/invoiceautotrans/invoicecorporate/cekPrint') ?>?InvoiceCorNo=${InvoiceCorNo}`)
					.then(status)
					.then(response => response.json())
					.then(response => {
						if (response.response < 1) {
							showSnackError('Data Invoice Corporate Kosong');
						} else {
							window.location.replace(`<?= base_url('finance/invoiceautotrans/invoicecorporate/cetak') ?>?InvoiceCorNo=${InvoiceCorNo}`);
						}
					});
			}
		});
