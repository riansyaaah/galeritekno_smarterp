...
<body>
	...
	<div id="app">
						...
						<div class="col-12">
							<div class="card">
								...
								<div class="card-body">
									...
									<button type="button" class="btn  btn-primary btn-sm mb-2" id="btn_add_official" onclick="add()" disabled><i class="fa fa-plus"></i> Add Official</button>
									<div class="table-responsive">
										<table class="table table-hover table-bordered" id="table-detail-official">
											<thead>
												<tr>
													<th>No</th>
													<th>Description</th>
			...
			<div class="modal-dialog modal-dialog-centered" role="document">
				<div class="modal-content">
					...
					<div class="modal-footer bg-whitesmoke br">
						<button class="btn btn-primary" type="button" id="btnOfficialBaru" onclick="return save()">
		...
		<script>
			...
			function add() {
				$('#status').val('tambah');
				document.getElementById("btnOfficialBaru").innerHTML = '<i class="fa fa-check"></i> SAVE';
				document.getElementById("btnOfficialBaru").disabled = false;
			...
			function edit(id, status) {
				...
				showLoading();
				$.ajax({
					url: '<?php echo base_url("hrm/payroll/officials/getOfficialById") ?>',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
			...
			function save() {
				var btn = document.getElementById("btnOfficialBaru");
				var code = $('#code').val();
				...
				if (description == "" || description == null || amount == "" || amount == null) {
					...
					$.ajax({
						url: '<?php echo base_url("hrm/payroll/officials/saveOfficial") ?>',
						type: 'POST',
						dataType: 'json',
						data: dataPost,
						success: function(res) {
				...
				$.ajax({
					...
					success: function(res) {
						...
						if (res.status_json) {
							...
							document.getElementById("headermodalhapus").innerHTML = "Delete Official ";
			...
			function itemHapus() {
				...
				$.ajax({
					url: '<?php echo base_url("hrm/payroll/officials/deleteOfficial") ?>',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
					success: function(res) {
			...
			function cariPeriode() {
				...
				$('#table-periode tbody').on('click', 'tr', function() {
					...
					// flash Table Official
					$('#table-detail-official').DataTable().clear().draw();
					$('#table-detail-official').DataTable().destroy();
					// $('#totalAmount').text('');
			...
			function OfficialCari() {
				...
				$("#table-detail-official").dataTable({
					paging: false,
					ordering: false,
					info: false,
					searching: false,
					destroy: true,
					ajax: {
						url: '<?= base_url("hrm/payroll/officials/getOfficialByStaffId") ?>',
						dataSrc: 'data',
						type: 'POST',
						dataType: 'json',
						data: dataPost,
