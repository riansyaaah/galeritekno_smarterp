...
<body>
	...
	<div id="app">
						...
						<div class="col-12">
							<div class="card">
								...
								<div class="card-body">
									...
									<button type="button" class="btn  btn-primary btn-sm mb-2" id="btn_add_incentive" onclick="add()" disabled><i class="fa fa-plus"></i> Add Incentive</button>
									<div class="table-responsive">
										<table class="table table-hover table-bordered" id="table-detail-incentive" style="width: 100%;">
											<thead>
												<tr>
													<th>No</th>
													<th>Description</th>
			...
			<div class="modal-dialog modal-dialog-centered" role="document">
				<div class="modal-content">
					...
					<div class="modal-footer bg-whitesmoke br">
						<button class="btn btn-primary" type="button" id="btnInsentifBaru" onclick="return save()">
		...
		<script>
			...
			function add() {
				$('#status').val('tambah');
				document.getElementById("btnInsentifBaru").innerHTML = '<i class="fa fa-check"></i> SAVE';
				document.getElementById("btnInsentifBaru").disabled = false;
				...
				document.getElementById("headermodaltambah").innerHTML = "ADD INCENTIVE";
			...
			function edit(id, status) {
				...
				showLoading();
				$.ajax({
					url: '<?php echo base_url("hrm/payroll/incentives/getIncentiveById") ?>',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
					success: function(res) {
						...
						if (res.status_json) {
							...
							$("#description").removeClass("is-invalid");
							$("#amount").removeClass("is-invalid");
							document.getElementById("headermodaltambah").innerHTML = "EDIT INCENTIVE ";
							document.getElementById("btnInsentifBaru").innerHTML = '<i class="fa fa-check"></i> SAVE';
							document.getElementById("btnInsentifBaru").disabled = false;
							document.getElementById("title_description").innerHTML = "DESCRIPTIOJ";
							document.getElementById("title_amount").innerHTML = "AMOUNT";
							dismisLoading();
							$("#AddModal").modal();
						} else {
			...
			function save() {
				var btn = document.getElementById("btnInsentifBaru");
				var code = $('#code').val();
				...
				if (description == "" || description == null || amount == "" || amount == null) {
					...
					$.ajax({
						url: '<?php echo base_url("hrm/payroll/incentives/saveIncentive") ?>',
						type: 'POST',
						dataType: 'json',
						data: dataPost,
						success: function(res) {
				...
				$.ajax({
					...
					success: function(res) {
						...
						if (res.status_json) {
							...
							document.getElementById("headermodalhapus").innerHTML = "Delete Incentive ";
			...
			function itemHapus() {
				...
				$.ajax({
					url: '<?php echo base_url("hrm/payroll/incentives/deleteIncentive") ?>',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
					success: function(res) {
			...
			function cariStaff() {
				...
				$('#table-staff tbody').on('click', 'tr', function() {
					...
					document.getElementById("btn_add_incentive").disabled = false;
			...
			function cariPeriode() {
				...
				$('#table-periode tbody').on('click', 'tr', function() {
					...
					document.getElementById("btn_add_incentive").disabled = true;
					document.getElementById("btn_cari_staff").disabled = false;

					$('#cariPeriode').modal('hide');

					// flash Table Incentive
					$('#table-detail-incentive').DataTable().clear().draw();
					$('#table-detail-incentive').DataTable().destroy();
					// $('#totalAmount').text('');
			...
			function InsentifCari() {
				...
				$("#table-detail-incentive").dataTable({
					paging: false,
					ordering: false,
					info: false,
					searching: false,
					destroy: true,
					ajax: {
						url: '<?= base_url("hrm/payroll/incentives/getIncentiveByStaffId") ?>',
						dataSrc: 'data',
						type: 'POST',
						dataType: 'json',
						data: dataPost,
