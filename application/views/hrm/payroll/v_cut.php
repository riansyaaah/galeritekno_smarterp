...
<body>
	...
	<div id="app">
						...
						<div class="col-12">
							<div class="card">
								...
								<div class="card-body">
									...
									<button type="button" class="btn  btn-primary btn-sm mb-2" id="btn_add_cut" onclick="add()" disabled><i class="fa fa-plus"></i> Add Cut</button>
									<div class="table-responsive">
										<table class="table table-bordered table-hover" id="table-detail-cut" style="width: 100%;">
											<thead>
												<tr>
													<th>No</th>
													<th>Description</th>
			...
			<div class="modal-dialog modal-dialog-centered" role="document">
				<div class="modal-content">
					...
					<div class="modal-footer bg-whitesmoke br">
						<button class="btn btn-primary" type="button" id="btnCutBaru" onclick="return save()">
		...
		<script>
			...
			function add() {
				$('#status').val('tambah');
				document.getElementById("btnCutBaru").innerHTML = '<i class="fa fa-check"></i> SAVE';
				document.getElementById("btnCutBaru").disabled = false;
			...
			function edit(id, status) {
				...
				showLoading();
				$.ajax({
					url: '<?php echo base_url("hrm/payroll/cuts/getCutById") ?>',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
			...
			function save() {
				var btn = document.getElementById("btnCutBaru");
				var code = $('#code').val();
				...
				if (description == "" || description == null || amount == "" || amount == null) {
					...
					$.ajax({
						url: '<?php echo base_url("hrm/payroll/cuts/saveCut") ?>',
						type: 'POST',
						dataType: 'json',
						data: dataPost,
						success: function(res) {
				...
				$.ajax({
					...
					success: function(res) {
						...
						if (res.status_json) {
							...
							document.getElementById("headermodalhapus").innerHTML = "Delete Cut ";
			...
			function itemHapus() {
				...
				$.ajax({
					url: '<?php echo base_url("hrm/payroll/cuts/deleteCut") ?>',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
					success: function(res) {
			...
			function cariStaff() {
				...
				$('#table-staff tbody').on('click', 'tr', function() {
					...
					document.getElementById("btn_add_cut").disabled = false;
			...
			function cariPeriode() {
				...
				$('#table-periode tbody').on('click', 'tr', function() {
					...
					document.getElementById("btn_add_cut").disabled = true;
					document.getElementById("btn_cari_staff").disabled = false;

					$('#cariPeriode').modal('hide');

					// flash Table Cut
					$('#table-detail-cut').DataTable().clear().draw();
					$('#table-detail-cut').DataTable().destroy();
					// $('#totalAmount').text('');
			...
			function CutCari() {
				...
				$("#table-detail-cut").dataTable({
					...
					destroy: true,
					ajax: {
						url: '<?= base_url("hrm/payroll/cuts/getCutByStaffId") ?>',
						dataSrc: 'data',
						type: 'POST',
						dataType: 'json',
						data: dataPost,
