							...
							<div class="card">
								<div class="card-header">
									<h4>
										<?=$title;?>
										<p></p>
										<button class="btn-sm btn-primary" onclick="add()"><i class="fa fa-plus"></i>
											Add</button>
									</h4>
									<hr>
								</div>
					...
					<div class="modal-body row">
						...
						<div class="col-md-6">
							<div class="form-group">
								<div class="section-title" id="title_penjamin"></div>
								<input type="text" class="form-control" id="namapenjamin" required="">
							</div>
						</div>
						<div class="col-sm-6">
							<div class="form-group">
		...
		<script>
			$(document).ready(function () {
				$("#table-menu").dataTable({
					ajax: {
						url: '<?php echo base_url("eklinik/masterdata/getPenjamin") ?>',
						dataSrc: 'data'
					},
					columns: [{
							"data": 'no',
			...
			function add(status) {
				...
				showLoading();
				$.ajax({
					url: '<?= base_url("eklinik/masterdata/getPenjamin") ?>',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
					success: function (res) {
						...
						if (res.status_json) {
							data = res.data;
							$('#status').val('tambah');
							document.getElementById("headermodaltambah").innerHTML = "ADD MASTER DATA PENJAMIN";
							document.getElementById("title_penjamin").innerHTML = "Nama Penjamin";
			...
			function saveitem() {
				...
				var id = $('#id').val();
				var namapenjamin = $('#namapenjamin').val();
				var keterangan = $('#keterangan').val();
				var status = $('#status').val();
				if (namapenjamin == "" || namapenjamin == null) {
					showSnackError("Harap isi Penjamin");
					...
					$.ajax({
						url: '<?= base_url("eklinik/masterdata/savePenjamin") ?>',
						type: 'POST',
						dataType: 'json',
						data: dataPost,
						success: function (res) {
			...
			function edit(id, status) {
				...
				showLoading();
				$.ajax({
					url: '<?= base_url("eklinik/masterdata/getPenjaminbyid") ?>',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
					success: function (res) {
						...
						if (res.status_json) {
							data = res.data;
							$('#id').val(data[0]['id']);
							$('#namapenjamin').val(data[0]['namapenjamin']);
							$('#keterangan').val(data[0]['keterangan']);
							$('#status').val('edit');
							document.getElementById("headermodaltambah").innerHTML = "EDIT MASTER DATA PENJAMIN";
							document.getElementById("title_penjamin").innerHTML = "Nama Penjamin";
							document.getElementById("title_keterangan").innerHTML = "Keterangan";
							dismisLoading();
							$("#tambahModal").modal();
				...
				$.ajax({
					...
					success: function (res) {
						...
						if (res.status_json) {
							...
							document.getElementById("headermodalhapus").innerHTML = "Delete Penjamin  ";
			...
			function itemHapus() {
				...
				$.ajax({
					url: '<?= base_url("eklinik/masterdata/deletePenjamin") ?>',
					type: 'POST',
					dataType: 'json',
					data: dataPost,
					success: function (res) {
