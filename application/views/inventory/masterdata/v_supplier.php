...
<body>
    ...
    <script>
        ...
        document.querySelector('#btn_save').addEventListener('click', e => {
            ...
            if(form.judul == 'Add') {
                (!form.nama || !form.email || !form.telp || !form.cp || !form.alamat)? showSnackError('Harap diisi') : postData(form);
            } else {
                (!form.idSupplier || !form.nama || !form.email || !form.telp || !form.cp || !form.alamat)? showSnackError('Harap diisi') : postData(form);
            }
        });
