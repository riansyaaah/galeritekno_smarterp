...
class Allowances extends CI_Controller
{
	...
	var $idMenu = "a1c9590e-ead9-43d2-a3f1-2ad74c57ef6e";
	...
	public function index()
	{
		...
		$this->load->view('hrm/master/v_allowance', $data);
	}
	...
	public function saveAllowance()
	{
		...
		try {
			...
			if ($post) {
				if ($status == 'tambah') {
					$check = $this->allowance->getAllowance(" where id = '$code' order by id limit 1");
					if (count($check) > 0) {
						$response['status_json'] = false;
