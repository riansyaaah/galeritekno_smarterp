...
class Contracts extends CI_Controller
{
	...
	function __Construct()
	{
		...
		$this->load->model('hrm/master/ModelContract', 'contract');
	}

	var $idMenu = "b03a0508-1372-43ae-9b93-c124bfc4df69";
	...
	public function index()
	{
		...
		$this->load->view('hrm/master/v_contract', $data);
	}
	...
	public function saveContract()
	{
		...
		try {
			...
			if ($post) {
				if ($status == 'tambah') {
					$check = $this->contract->getContract(" where id = '$code' order by id limit 1");
					if (count($check) > 0) {
						$response['status_json'] = false;
